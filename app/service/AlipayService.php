...
class AlipayService
{
     ...
     * @param array $request
     */
    public static function webPay(array $request)
    {
        $data = array(
        ...
        $data["sign"] = self::generateSign($data, $data['sign_type']);
        echo self::buildRequestForm($data);
    }
     ...
     * @return mixed
     */
    public static function qrcodePay(array $request)
    {
        $data = array(
            'app_id' => Config::get("alipay")['app_id'],
        ...
        if ($response_arr['alipay_trade_precreate_response']) {
            ...
            if (self::verify($data_str, $response_arr['sign'], '', 'RSA2')) {
    ...
    //用公钥解密--验证签名是否正确
    protected static function verify($data, $sign, $rsaPublicKeyFilePath = '', $signType = 'RSA')
    {
     ...
     * @return bool
     */
    public static  function checkNoticeSign($params)
    {
        $sign = $params['sign'];
        $params['sign_type'] = null;
        $params['sign'] = null;
        return self::verify(self::getSignContent($params), $sign,'','RSA2');
    }
    ...
    protected  static function generateSign($params, $signType = "RSA")
    {
