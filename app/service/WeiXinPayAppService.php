...
class WeiXinPayAppService
{
     ...
     * @return array
     */
    public static  function qrcodePay($request)
    {
    ...
    private static function unifiedOrder(Array &$request, $timeOut = 6)
    {
        ...
        $request['appid'] = Config::get("weixin_app")['appid'];
        $request['mch_id'] = Config::get("weixin_app")['mchid'];
...
//        $xml = $this->toXml($request);
        ...
        $response = HttpUtil::curl($url, $xml, $timeOut, function ($ch) {
            ...
            curl_setopt($ch, CURLOPT_SSLCERT, Config::get("weixin_app")["sslcert_path"]);
            curl_setopt($ch, CURLOPT_SSLKEYTYPE, 'PEM');
            curl_setopt($ch, CURLOPT_SSLKEY, Config::get("weixin_app")['sslkey_path']);
     ...
     */
    private static function makeSign($request)
    {
        //签名步骤一：按字典序排序参数
        ksort($request);
        $string = self::toUrlParams($request);
        //签名步骤二：在string后加入KEY
        $string = $string . "&key=" . Config::get("weixin_app")["key"];
        //签名步骤三：MD5加密
        $string = md5($string);
        //签名步骤四：所有字符转为大写
